...
const FeaturedRent = ({ posts }) => {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        ...
        const filteredProperties = res.data.filter((property) => property.type === "rent" && property.rentstatus );
  ...
  return (
    <div className="featured-properties-rent">
      <h2 className="section-title">Dự án bất động sản cho thuê</h2>
            ...
            <div className="property-card" key={property.id}>
              ...
              <span
                  className={`status-label ${
                    property.rentstatus === "raoban"
                      ? "approved"
                      : property.rentstatus === "danggiaodich"
                      ? "pending"
                      : "look"
                  }`}
                >
